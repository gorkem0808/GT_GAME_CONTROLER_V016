...
APP_TITLE = "GT GAME CONTROLER v022"
...
class SerialDevice:
    def __init__(self, port, hello=""):
        ...
        self.p1_coin_pin = 17
        self.p2_coin_pin = 21
        self.p1_trigger_pin = 7
        self.p2_trigger_pin = 11
    ...
    def parse_config_from_text(self, line):
        # Hem CONFIG,RELAY,HIGH,... hem de STATUS içindeki CFG,RELAY:HIGH,... biçimini okur.
        ...
        try:
            for i, p in enumerate(parts):
                ...
                if p == "TRIGGERPINS" and i+2 < len(parts):
                    self.p1_trigger_pin = int(parts[i+1]); self.p2_trigger_pin = int(parts[i+2])
        except Exception:
            pass
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.p1_trigger_gp = tk.StringVar(value="GP7")
        self.p2_trigger_gp = tk.StringVar(value="GP11")
        self.controller_fields_dirty = False
        self.controller_loaded_once = False
        self.config_apply_lock_until = 0
        self.expected_cfg = None
        self.setup_style()
        self.create_ui()
    ...
    def create_ui(self):
        ...
        ttk.Label(header, text="v022  |  Temiz controller + seçilebilir coin/tetik/röle GP", style="Sub.TLabel").pack(side="left", padx=16)
    ...
    def make_controller_tab(self):
        ...
        ttk.Label(top,text="P1 tetik GP:").grid(row=3,column=0,sticky="e",padx=5,pady=5)
        self.cb_p1_trigger_gp = ttk.Combobox(top,textvariable=self.p1_trigger_gp,values=GP_CHOICES,width=10,state="readonly")
        self.cb_p1_trigger_gp.grid(row=3,column=1,sticky="w")
        ttk.Label(top,text="P2 tetik GP:").grid(row=3,column=2,sticky="e",padx=5)
        self.cb_p2_trigger_gp = ttk.Combobox(top,textvariable=self.p2_trigger_gp,values=GP_CHOICES,width=10,state="readonly")
        self.cb_p2_trigger_gp.grid(row=3,column=3,sticky="w")
        ...
        ttk.Button(top,text="Controller Config Oku",command=self.read_controller_config).grid(row=0,column=6,rowspan=4,padx=10,sticky="ns")
        for cb in [self.cb_relay_mode,self.cb_p1_coin,self.cb_p2_coin,self.cb_p1_relay,self.cb_p2_relay,self.cb_p1_coin_gp,self.cb_p2_coin_gp,self.cb_p1_trigger_gp,self.cb_p2_trigger_gp]:
    ...
    def apply_controller_values_from_device(self, kb, force=False):
        # V020: Kaydet tuşundan sonra eski STATUS satırı gelirse combobox'ları geri çevirmesin.
        if time.time() < self.config_apply_lock_until and not force:
            return
        ...
        self.p1_trigger_gp.set(f"GP{getattr(kb, 'p1_trigger_pin', 7)}")
        self.p2_trigger_gp.set(f"GP{getattr(kb, 'p2_trigger_pin', 11)}")
    ...
    def send_cfg(self):
        ...
        t1 = self.gp_num(self.p1_trigger_gp.get())
        t2 = self.gp_num(self.p2_trigger_gp.get())
        used = [p1, p2, c1, c2, t1, t2]
        if len(set(used)) != len(used):
            messagebox.showwarning("Hatalı pin", "P1/P2 röle, coin ve tetik pinleri aynı olamaz."); return
        ...
        # V020: Kullanıcı GP3 seçip Kaydet dediği anda ekrandaki değer kilitlenir.
        # Controller yeni CONFIG dönene kadar eski GP17 STATUS satırı ekrana geri yazamaz.
        self.controller_fields_dirty = True
        self.config_apply_lock_until = time.time() + 1.8
        self.expected_cfg = (p1, p2, c1, c2, t1, t2, self.relay_mode.get(), self.p1_coin.get(), self.p2_coin.get())

        cmds = [
            f"SETCFG,{self.relay_mode.get()},{self.p1_coin.get()},{self.p2_coin.get()},{p1},{p2},{c1},{c2},{t1},{t2}",
            f"RELAYPIN,1,{p1}",
            f"RELAYPIN,2,{p2}",
            f"COINPIN,1,{c1}",
            f"COINPIN,2,{c2}",
        ...
        for cmd in cmds:
            d.send(cmd)
            time.sleep(0.06)
        self.ctrl_status.set(f"Kaydediliyor: P1 coin GP{c1}, P2 coin GP{c2}, P1 tetik GP{t1}, P2 tetik GP{t2}, P1 röle GP{p1}, P2 röle GP{p2}")
        self.after(500, lambda: self.controller_send("GETCFG"))
        self.after(1500, self.verify_controller_config)

    def verify_controller_config(self):
        d = self.get_ctrl()
        if not d or not self.expected_cfg:
            self.controller_fields_dirty = False
            return
        p1, p2, c1, c2, t1, t2, rm, pc1, pc2 = self.expected_cfg
        ok = (d.p1_relay_pin == p1 and d.p2_relay_pin == p2 and
              getattr(d, 'p1_coin_pin', 17) == c1 and getattr(d, 'p2_coin_pin', 21) == c2 and
              getattr(d, 'p1_trigger_pin', 7) == t1 and getattr(d, 'p2_trigger_pin', 11) == t2)
        self.controller_fields_dirty = False
        self.config_apply_lock_until = 0
        if ok:
            self.ctrl_status.set(f"✅ Ayar kaydedildi: P1 coin GP{c1}, P2 coin GP{c2}, P1 tetik GP{t1}, P2 tetik GP{t2}, P1 röle GP{p1}, P2 röle GP{p2}")
            self.apply_controller_values_from_device(d, force=True)
        else:
            messagebox.showwarning("Ayar kaydedilmedi",
                "Program ayarı gönderdi ama Controller Pico eski değeri geri bildirdi.\n\n"
                "Bu genelde Controller Pico'da eski UF2 yüklü demektir. Yeni V022 controller.uf2 derlenip yüklenmeli.")
    ...
    def refresh_ui(self):
        ...
        if kb:
            ...
            self.relay_text.insert("end",f"P1 tetik GP{getattr(kb, 'p1_trigger_pin', 7)} | P2 tetik GP{getattr(kb, 'p2_trigger_pin', 11)}\n")
