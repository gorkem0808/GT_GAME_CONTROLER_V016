...
APP_TITLE = "GT GAME CONTROLER v020"
...
class App(tk.Tk):
    ...
    def create_ui(self):
        ...
        ttk.Label(header, text="v020  |  V019 korunur + ayar kaydetme sabitlendi", style="Sub.TLabel").pack(side="left", padx=16)
    ...
    def send_cfg(self):
        ...
        self.expected_cfg = (p1, p2, c1, c2, self.relay_mode.get(), self.p1_coin.get(), self.p2_coin.get())
        ...
        for cmd in cmds:
            d.send(cmd)
            time.sleep(0.06)
        self.ctrl_status.set(f"Kaydediliyor: P1 coin GP{c1}, P2 coin GP{c2}, P1 röle GP{p1}, P2 röle GP{p2}")
    ...
    def verify_controller_config(self):
        ...
        if not d or not self.expected_cfg:
            self.controller_fields_dirty = False
            return
        p1, p2, c1, c2, rm, pc1, pc2 = self.expected_cfg
        ok = (d.p1_relay_pin == p1 and d.p2_relay_pin == p2 and
              getattr(d, 'p1_coin_pin', 17) == c1 and getattr(d, 'p2_coin_pin', 21) == c2)
        self.controller_fields_dirty = False
        self.config_apply_lock_until = 0
        if ok:
            self.ctrl_status.set(f"✅ Ayar kaydedildi: P1 coin GP{c1}, P2 coin GP{c2}, P1 röle GP{p1}, P2 röle GP{p2}")
